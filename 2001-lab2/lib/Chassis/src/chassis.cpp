...
Twist Chassis::CalcOdomFromWheelMotion(void)
{
     ...
     */
    // measures ticks or how much the robot moved in a controlled time
    float left_ticks  = leftMotor.speed;
    float right_ticks = rightMotor.speed;

    // converts ticks to distance in cm
    float left_cm  = left_ticks  / LEFT_TICKS_PER_CM;
    float right_cm = right_ticks / RIGHT_TICKS_PER_CM;
    ...
    float vl = left_cm / dt;
    float vr = right_cm / dt;

    // u = how fast its moving forward in cm/s
    velocity.u     = (vr + vl) / 2.0f;        
    //omega = how fast the robot is turning in rad/s         
    velocity.omega = (vr - vl) / (2.0f * ROBOT_RADIUS); 
